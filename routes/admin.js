...
router.post("/settings", async (req, res) => {
  try {
    const {
      adRewardApple, dailyAdLimit, referralBonusApple, referralCommissionRate,
      referralTasksRequired, activeAdNetwork, showPaymentBanner, showWithdrawButton, showLeaderboard,
    } = req.body;
    const settings = await getSettings();
    ...
    if (activeAdNetwork !== undefined && ["monetag", "gigapub", "both", "none"].includes(activeAdNetwork)) {
      settings.activeAdNetwork = activeAdNetwork;
    }
    if (showPaymentBanner !== undefined) settings.showPaymentBanner = !!showPaymentBanner;
    if (showWithdrawButton !== undefined) settings.showWithdrawButton = !!showWithdrawButton;
    if (showLeaderboard !== undefined) settings.showLeaderboard = !!showLeaderboard;
