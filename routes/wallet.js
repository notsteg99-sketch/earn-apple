...
const WITHDRAW_FEE_RATE = Number(process.env.WITHDRAW_FEE_RATE || 0.2); // 20% network/processing fee
...
// Frontend isse call karega jab user amount type kare, taaki fee live dikha sake
...
router.post("/withdraw", telegramAuth, async (req, res) => {
  try {
    ...
    const withdrawal = await Withdrawal.create({
      telegramId: user.telegramId,
      amountUsdt: netAmount, // amount that will actually be sent, after fee
